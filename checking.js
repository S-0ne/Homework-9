...
function enterPersonalInfo() {
    let userInputName = prompt("Введіть своє прізвище та ім'я через пробіл: ");
    ...
    let userInputEmail = prompt("Введіть свою електронну пошту: ");
    while (!validateEmail(userInputEmail)) {
        console.log("Будь ласка, введіть коректний формат електронної пошти.");
        userInputEmail = prompt("Введіть свою електронну пошту: ");
    }
    persons.email = userInputEmail;

    let userInputPhoneNumber = prompt("Введіть ваш номер телефону (формат: +380XXXXXXXXX): ");
    while (!validatePhoneNumber(userInputPhoneNumber)) {
        ...
        userInputPhoneNumber = prompt("Введіть свій номер телефону (формат: +380XXXXXXXXX): ");
    }
    persons.phonenumber = userInputPhoneNumber;

    let userInputBirthdate = prompt("Введіть ваш день народження через пробіл (день місяць рік): ");
    while (!validateDate(userInputBirthdate)) {
        console.log("Будь ласка, введіть коректний формат дати народження (день місяць рік).");
        userInputBirthdate = prompt("Введіть свій день народження через пробіл (день місяць рік): ");
    }
